...
// Function to load and run an ELF binary
void load_and_run_elf(char **exe)
{
    elf_file = exe[1];
...
// Function to handle temporary page faults
void handle_temp_page_fault(siginfo_t *info)
{
    ...
    for (int i = 0; i < new_ehdr->e_phnum; i++)
    {
        ...
        if (ls < 0 || red < 0)
        {
            printf("Error in seeking the pointer and reading data !!!!\n");
            exit(1);
        }
        if (new_phdr->p_vaddr + new_phdr->p_memsz > fault_address && fault_address >= new_phdr->p_vaddr)
        {
            ...
            void *virtual_mem = mmap((void *)new_virtual_address, new_memory, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, 0, 0);
            //printf("167\n");
            ...
            int a = read(fd, virtual_mem, new_memory);
            if (a == 0 || a == -1)
            {
                printf("EOF Reached!!!!\n");
                exit(1);
...
int number_of_pages_to_be_allocated(Elf32_Word mem_size_segment)
{
    ...
    total_page_allocated += number_of_pages; // Track total pages allocated
